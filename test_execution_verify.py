...
def prepare_test_input(test_case: TestCase) -> tuple[list[str], Optional[str]]:
...
def execute_operations_test(
    module, test_case: TestCase, start_time: float
) -> TestResult:
    """Execute operation-based tests for classes like LRUCache."""
    import time

    input_data = test_case.input_data
    capacity = input_data.get("capacity", 2)
    operations = input_data.get("operations", [])

    # Find the class to instantiate
    classes = {name: obj for name, obj in vars(module).items() if isinstance(obj, type)}

    if not classes:
        return TestResult(
            test_id=test_case.test_id,
            passed=False,
            actual_output="No class found",
            expected_output=test_case.expected_output,
            execution_time=time.time() - start_time,
        )

    # Try to find LRUCache or first available class
    class_name = "LRUCache" if "LRUCache" in classes else list(classes.keys())[0]
    ...
    try:
        ...
        error_passed = verify_output(typed_error, test_case.expected_output, "error")
        return TestResult(
            test_id=test_case.test_id,
            passed=error_passed,
            actual_output=typed_error,
            expected_output=test_case.expected_output,
            error_message=None
            if error_passed
            else f"Failed to instantiate {class_name}: {e}",
            execution_time=time.time() - start_time,
        )
    ...
    for op in operations:
        ...
        try:
            if op_type == "put":
                ...
                get_results.append(res if res is not None else -1)
        except Exception as e:
            get_results.append(f"ERROR: {e}")

    if not get_results:
        actual_output = "[]"
    else:
        actual_output = str(get_results)

    expected_output = test_case.expected_output
    try:
        import ast

        parsed_expected = ast.literal_eval(expected_output)
        if isinstance(parsed_expected, list):
            get_count = sum(1 for op in operations if op.get("op", "").lower() == "get")
            normalized_expected = parsed_expected
            if len(normalized_expected) > get_count:
                normalized_expected = [
                    v for v in normalized_expected if not isinstance(v, bool)
                ]
                while (
                    len(normalized_expected) > get_count and None in normalized_expected
                ):
                    normalized_expected.remove(None)
                if len(normalized_expected) > get_count:
                    normalized_expected = normalized_expected[:get_count]
            normalized_expected = [
                (-1 if v is None else v) for v in normalized_expected
            ]
            expected_output = str(normalized_expected)
    except (ValueError, SyntaxError):
        expected_output = test_case.expected_output

    passed = verify_output(actual_output, expected_output, test_case.test_type)
...
def execute_as_function(
    main_file: Path, test_case: TestCase, timeout: int = 10
) -> Optional[TestResult]:
    ...
    try:
        ...
        # Handle operation-based tests for classes like LRUCache
        if "operations" in input_data and "capacity" in input_data:
            return execute_operations_test(module, test_case, start_time)
        ...
        passed = verify_output(
            actual_output, test_case.expected_output, test_case.test_type
        )
...
def execute_as_cli(
    main_file: Path, test_case: TestCase, timeout: int, start_time: float
) -> TestResult:
...
def compare_json_outputs(actual: Any, expected: Any, tolerance: float = 0.01) -> bool:
    ...
    if isinstance(actual, dict):
        ...
        if len(actual) != len(expected):
            return False
        return all(
            compare_json_outputs(a, e, tolerance) for a, e in zip(actual, expected)
        )
...
def run_all_tests(
    test_cases: list[TestCase], main_file: Optional[Path] = None
) -> list[TestResult]:
...
def print_summary(results: list[TestResult]) -> None:
    ...
    if failed > 0:
        ...
        for result in results:
            if not result.passed:
                print(
                    f"  - {result.test_id}: {result.error_message or 'Output mismatch'}"
                )
